...
import { 
  ...
  Banknote, Landmark, Lock, FileText, User, Mailbox // <-- Added Mailbox
} from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    town: "",
    area: "",
    postal_address: "", // <-- Added postal address
    kra_pin: "", 
    settlement_name: "", 
    existingLogoUrl: "", 
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // <-- Added Real-time Slug formatting
    if (name === "storeSlug") {
      value = value.toLowerCase().replace(/[^a-z0-9-]/g, '-');
    }

    // <-- Added Real-time Title Casing
  ...
  const isFormValid = 
    ...
    formData.postal_address.trim() !== "" && // <-- Added Postal address requirement
    ...
    const fetchStoreDetails = async () => {
      try {
        ...
        const { data: store, error } = await supabase
          .from("stores")
          // <-- Added postal_address to selection
        ...
        if (store) {
          ...
          setFormData({
            ...
            town: store.town || "",
            area: store.area || "",
            postal_address: store.postal_address || "", // <-- Mapping new column
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updates = {
        ...
        slug: slugifiedName, // <-- Using the potentially formatted slug
        description: formData.description,
        category: formData.category, 
        county: formData.county,
        town: formData.town,
        area: formData.area,
        postal_address: formData.postal_address, // <-- Saving postal address
          ...
          <div className="grid md:grid-cols-2 gap-5 sm:gap-6">
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  type="text" required value={formData.storeName} name="storeName" // <-- Added name
                  onChange={handleInputChange} onBlur={handleBlur} // <-- Updated handler
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select 
                  required name="category" // <-- Added name
          ...
          <div className="grid md:grid-cols-2 gap-5 sm:gap-6">
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  type="text" value={formData.storeSlug} name="storeSlug" // <-- Added name
                  onChange={handleInputChange} // <-- Updated handler
                  className="w-full pl-11 pr-4 py-3 sm:py-3.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-base sm:text-sm bg-slate-50 focus:bg-white" 
                  placeholder="e.g. the-coffee-house" 
                />
            ...
            <div>
              ...
              <div className="relative">
                ...
                <textarea 
                  rows={3} value={formData.description} name="description" // <-- Added name
                  onChange={handleInputChange} onBlur={handleBlur} // <-- Updated handler
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    type="text" required value={formData.county} name="county" // <-- Added name
                    onChange={handleInputChange} // <-- Updated handler
                    className="w-full pl-11 pr-4 py-3 sm:py-3.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-base sm:text-sm bg-slate-50 focus:bg-white" 
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    type="text" required value={formData.town} name="town" // <-- Added name
                    onChange={handleInputChange} // <-- Updated handler
                    className="w-full pl-11 pr-4 py-3 sm:py-3.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-base sm:text-sm bg-slate-50 focus:bg-white" 
              ...
              <div className="sm:col-span-2 lg:col-span-1">
                ...
                <div className="relative">
                  ...
                  <input 
                    type="text" required value={formData.area} name="area" // <-- Added name
                    onChange={handleInputChange} // <-- Updated handler
                    className="w-full pl-11 pr-4 py-3 sm:py-3.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all text-base sm:text-sm bg-slate-50 focus:bg-white" 
        ...
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-5 sm:p-8 space-y-6 sm:space-y-8">
          <div className="border-b border-slate-100 pb-4 flex justify-between items-start">
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                {/* <-- Updated hint message when locked --> */}
          ...
          <div className="grid md:grid-cols-2 gap-5 sm:gap-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  name="kra_pin" 
                  required
                  disabled={isFinancialsLocked} // <-- Added Lock
                  value={formData.kra_pin} 
                  onChange={handleInputChange} 
                  onBlur={handleBlur}
                  className={`w-full pl-10 pr-4 py-2.5 border rounded-xl outline-none transition-all text-sm uppercase ${isFinancialsLocked ? 'bg-slate-100 border-slate-200 text-slate-500 cursor-not-allowed' : errors.kra_pin ? 'border-red-500 bg-red-50 focus:ring-red-500' : 'border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-emerald-500'}`} // <-- Added Locked Styling
            ...
            <div>
              ...
              <div className="flex gap-4"> {/* <-- Removed wrap, fixed padding for better alignment */}
          ...
          <div className={`p-5 sm:p-6 rounded-3xl border transition-colors ${isFinancialsLocked ? 'bg-slate-50 border-slate-200' : 'bg-blue-50/30 border-blue-100'}`}>
            ...
            {/* Payout Toggle */}
            <div className="flex flex-wrap gap-4 max-w-md mb-6"> {/* <-- Adjusted layout */}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    onChange={handleInputChange} 
                    onBlur={handleBlur}
                    className={`w-full pl-10 pr-4 py-2.5 border rounded-xl outline-none transition-all text-sm ${isFinancialsLocked ? 'bg-slate-100 border-slate-200 text-slate-500 cursor-not-allowed' : errors.settlement_name ? 'border-red-500 bg-red-50 focus:ring-red-500' : 'border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-emerald-500'}`} // <-- Added Locked Styling
              ...
              {formData.payout_method === 'BANK' && (
                <div className="animate-in fade-in zoom-in duration-200">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      type="text" name="payout_bank_code" // <-- Added name
                      required={formData.payout_method === 'BANK'} 
                      disabled={isFinancialsLocked} // <-- Added Lock
                      value={formData.payout_bank_code} 
                      onChange={handleInputChange} // <-- Updated Handler
                      className={`w-full pl-11 pr-4 py-3 sm:py-3.5 border rounded-xl outline-none transition-all text-base sm:text-sm ${isFinancialsLocked ? 'bg-slate-100 border-slate-200 text-slate-500 cursor-not-allowed' : 'bg-white border-slate-300 focus:ring-2 focus:ring-blue-500'}`} // <-- Added Locked Styling
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    onChange={handleInputChange} 
                    onBlur={handleBlur}
                    className={`w-full pl-10 pr-4 py-2.5 border rounded-xl outline-none transition-all text-sm ${isFinancialsLocked ? 'bg-slate-100 border-slate-200 text-slate-500 cursor-not-allowed' : errors.payout_account_number ? 'border-red-500 bg-red-50 focus:ring-red-500' : 'border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-emerald-500'}`} // <-- Added Locked Styling
