              ...
              {stores.map((store) => {
                ...
                >
                  ...
                  <div className="relative h-44 w-full bg-slate-100 overflow-hidden shrink-0">
                    ...
                    {/* Top Badges: YOUR Category & VIP */}
                    <div className="absolute top-3 left-3 right-3 flex justify-between items-start z-10">
                      {store.category ? (
                        <span className="bg-white/90 backdrop-blur-sm text-slate-800 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-md shadow-sm truncate max-w-[120px]">
                          {store.category}
                        </span>
                      ) : <div></div>}

                      {store.tier === "VIP" && (
                        <span className="bg-gradient-to-r from-yellow-400 to-amber-500 text-white text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md shadow-sm flex items-center gap-1">
                          <Star className="w-3 h-3 fill-white" /> VIP
                        </span>
                      )}
                    </div>

                    {/* Bottom Overlay: YOUR Store Name */}
                  ...
                  <div className="p-4 sm:p-5 flex-1 flex flex-col bg-white relative z-20">
                    ...
                    {/* YOUR Exact Location Logic */}
                    {locationText && (
                      <p className="text-emerald-600 text-xs sm:text-sm font-bold mb-3 flex items-center gap-1.5 line-clamp-1 shrink-0">
                        <MapPin className="h-4 w-4 shrink-0" /> {locationText}
                      </p>
                    )}
                    ...
                    {/* YOUR Exact CTA Logic (Locked to bottom) */}
                    <div className="mt-auto pt-4 w-full shrink-0 border-t border-slate-50">
                      <div className="w-full flex items-center justify-between text-slate-700 font-bold group-hover:text-emerald-600 transition-colors">
                        <div className="flex items-center gap-2">
                          <CtaIcon className="h-4 w-4" />
                          <span className="text-sm">{ctaLabel}</span>
                        </div>
                        <div className="h-8 w-8 rounded-full bg-slate-50 group-hover:bg-emerald-50 flex items-center justify-center transition-colors border border-slate-100 group-hover:border-emerald-200">
